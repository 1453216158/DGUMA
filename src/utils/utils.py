...
def contrastLoss(embeds1, embeds2, nodes,config):
	embeds1 = F.normalize(embeds1, p=2)
	embeds2 = F.normalize(embeds2, p=2)
	pckEmbeds1 = embeds1[nodes]
	pckEmbeds2 = embeds2[nodes]
	nume = t.exp(t.sum(pckEmbeds1 * pckEmbeds2, dim=-1) / config['temp'])
	deno = t.exp(pckEmbeds1 @ embeds2.T / config['temp']).sum(-1)
...
def build_sim(context):
    """
    构建自相似矩阵（Self-similarity Matrix）

    Args:
        context (torch.Tensor): 输入特征矩阵，形状为 [N, D]，N为样本数，D为特征维度

    Returns:
        torch.Tensor: 相似度矩阵，形状为 [N, N]

    功能说明：
    1. 对输入矩阵进行L2归一化（行归一化）
    2. 计算归一化后的矩阵与其转置的乘积，得到样本间的相似度矩阵
    # L2归一化：将每个特征向量除以它的L2范数
    """
    norm_squared = torch.sum(context ** 2, dim=1)
    dot_product = torch.mm(context, context.T)
    denominator = norm_squared.unsqueeze(1) + norm_squared.unsqueeze(0) - dot_product
    jaccard_sim = dot_product / (denominator + 1e-8)
    return jaccard_sim
