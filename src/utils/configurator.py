...
class Config(object):
    ...
    def _load_dataset_model_config(self, config_dict):
        ...
        file_list.append(os.path.join(cur_dir, "model", "{}.yaml".format(config_dict['model'].upper())))
    ...
    def _init_device(self):
        ...
        if use_gpu:
            ...
            self.final_config_dict['device'] = torch.device(f"cuda:{gpu_id}")
        else:
            self.final_config_dict['device'] = torch.device("cpu")
        print(f"Using device: {self.final_config_dict['device']}")
