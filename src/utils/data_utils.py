...
def load_data_with_user_sampling(file_path, user_limit=None):
    """
    加载数据并控制用户数量

    Args:
        file_path: 数据文件路径
        user_limit: 用户数量上限，None表示不限制
    """
    data = pd.read_csv(file_path, sep='\t')

    if user_limit is not None:
        all_users = data['userID'].unique().tolist()
        if len(all_users) > user_limit:
            sampled_users = random.sample(all_users, user_limit)
            data = data[data['userID'].isin(sampled_users)]

            user_map = {user_id: i for i, user_id in enumerate(sorted(data['userID'].unique()))}
